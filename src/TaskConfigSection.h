...
class TaskConfigSection : public ConfigSection {
 public:
  ...
  // Copy constructor: clones all settings from another task
  TaskConfigSection(const TaskConfigSection& other);
  ~TaskConfigSection();
  ...
  int GetGriddedOutputs() {
    return griddedOutputs;
  }

  // CSV parameter sweep support
  bool HasParamSweep() const {
    return paramSweepSet;
  }
  const char* GetParamSweepCSV() const {
    return paramSweepCSV;
  }

  // Setters for sweep task overrides
  void SetName(const char* n) {
    strncpy(name, n, CONFIG_MAX_LEN - 1);
    name[CONFIG_MAX_LEN - 1] = '\0';
  }
  void SetOutput(const char* o) {
    strncpy(output, o, CONFIG_MAX_LEN - 1);
    output[CONFIG_MAX_LEN - 1] = '\0';
  }
  void SetParamsSec(ParamSetConfigSection* p) {
    params = p;
  }
  void SetRoutingParamsSec(RoutingParamSetConfigSection* rp) {
    paramsRouting = rp;
  }
 ...
 private:
  bool styleSet, modelSet, basinSet, precipSet, qpfSet, petSet, outputSet, stateSet, paramsSet,
      timestepSet, timeStateSet, timeBeginSet, timeWarmEndSet, timeEndSet, caliParamSet,
      routingParamsSet, routingCaliParamSet, defaultParamsSet, routeSet;
  ...
  bool paramSweepSet;
  char paramSweepCSV[CONFIG_MAX_LEN];
  ...
  char actionGrid[CONFIG_MAX_LEN], minorGrid[CONFIG_MAX_LEN], moderateGrid[CONFIG_MAX_LEN],
      majorGrid[CONFIG_MAX_LEN];
  char actionSDGrid[CONFIG_MAX_LEN], minorSDGrid[CONFIG_MAX_LEN], moderateSDGrid[CONFIG_MAX_LEN],
      majorSDGrid[CONFIG_MAX_LEN];
