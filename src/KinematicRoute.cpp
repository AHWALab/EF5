...
bool KWRoute::InitializeModel(std::vector<GridNode>* newNodes,
...
void KWRoute::InitializeStates(TimeVar* beginTime, char* statePath, std::vector<float>* fastFlow,
...
bool KWRoute::Route(float stepHours, std::vector<float>* fastFlow, std::vector<float>* slowFlow,
...
void KWRoute::RouteInt(float stepSeconds, GridNode* node, KWGridNode* cNode, float fastFlow,
                       float slowFlow) {
  if (!cNode->channelGridCell) {
    float beta = 0.6;
    ...
    if (cNode->incomingWaterOverland + cNode->states[STATE_KW_PQ] > 0.0) {
      backDiffq =
          pow((cNode->incomingWaterOverland + cNode->states[STATE_KW_PQ]) / 2.0, beta - 1.0);
    ...
    if (node->downStreamNode != INVALID_DOWNSTREAM_NODE &&
        !kwNodes[nodes->at(node->downStreamNode).modelIndex].daActive) {
    ...
    if (cNode->routeCNode[0][KW_LAYER_INTERFLOW]) {
      double interflowLeak0 = interflowLeak * cNode->routeAmount[0][KW_LAYER_INTERFLOW] *
    ...
    if (cNode->routeCNode[1][KW_LAYER_INTERFLOW]) {
      double interflowLeak1 = interflowLeak * cNode->routeAmount[1][KW_LAYER_INTERFLOW] *
    ...
    if (cNode->incomingWaterOverland + cNode->states[STATE_KW_PO] > 0.0) {
      backDiffq =
          pow((cNode->incomingWaterOverland + cNode->states[STATE_KW_PO]) / 2.0, beta - 1.0);
      if (!std::isfinite(backDiffq)) {
        backDiffq = 0.0;
      }
    }
...
void KWRoute::InitializeParameters(std::map<GaugeConfigSection*, float*>* paramSettings,
                                   std::vector<FloatGrid*>* paramGrids) {
  // This pass distributes parameters
  size_t numNodes = nodes->size();
  size_t unused = 0;

  // if (paramGrids->at(PARAM_KINEMATIC_ALPHA) &&
  // g_DEM->IsSpatialMatch(paramGrids->at(PARAM_KINEMATIC_ALPHA)))
  // {
  //     printf("Alpha grid is match!\n");
  // }

  // if (paramGrids->at(PARAM_KINEMATIC_BETA) &&
  // g_DEM->IsSpatialMatch(paramGrids->at(PARAM_KINEMATIC_BETA)))
  // {
...
void KWRoute::InitializeRouting(float timeSeconds) {
  // This pass distributes parameters & calculates the time it takes for water to cross the grid
  // cell.
  size_t numNodes = nodes->size();
  for (size_t i = 0; i < numNodes; i++) {
    GridNode* node = &nodes->at(i);
