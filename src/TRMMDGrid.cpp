...
FloatGrid* ReadFloatTRMMDGrid(char* file, FloatGrid* grid) {
  ...
  if (!grid) {
    ...
    if (!grid->data) {
      WARNING_LOGF("TRMM Daily file %s too large (out of memory) with %li rows", file,
                   grid->numRows);
      delete grid;
      gzclose(fileH);
      return NULL;
    ...
    for (long i = 0; i < grid->numRows; i++) {
      ...
      if (!grid->data[i]) {
        WARNING_LOGF("TRMM Daily file %s too large (out of memory) with %li cols", file,
                     grid->numCols);
        delete grid;
        gzclose(fileH);
        return NULL;
  ...
  for (long i = 0; i < grid->numRows; i++) {
    if (gzread(fileH, shortData, (unsigned int)(sizeof(float) * grid->numCols)) !=
