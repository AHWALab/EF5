...
bool Snow17Model::SnowBalance(float jday, float stepHours, std::vector<float>* precip,
...
void Snow17Model::SnowBalanceInt(GridNode* node, Snow17GridNode* cNode, float stepHours, float jday,
  ...
  float mf = (stepHours / 6) *
             ((Sv * (cNode->params[PARAM_SNOW17_MFMAX] - cNode->params[PARAM_SNOW17_MFMIN])) +
  ...
  float Pn =
      precip * fracsnow * cNode->params[PARAM_SNOW17_SCF];  // water equivalent of new snowfall (mm)
  ...
  if (Pn > (1.5 * stepHours)) {
    ...
    cNode->states[STATE_SNOW17_ATI] =
        cNode->states[STATE_SNOW17_ATI] +
        TIPM_dtt * (tempIn - cNode->states[STATE_SNOW17_ATI]);  // Antecedent temperature index
  }
  ...
  float delta_HD_T =
      cNode->params[PARAM_SNOW17_NMF] * (stepHours / 6.0) *
      (mf / cNode->params[PARAM_SNOW17_MFMAX]) *
      (cNode->states[STATE_SNOW17_ATI] - T_snow_new);  // delta_HD_T = change in heat deficit due to
  ...
  float e_sat =
      2.7489 * 100000000.0 *
      expf((-4278.63 / (tempIn + 242.792)));  // saturated vapor pressure at T_air_meanC (mb)
  ...
  float Qw = Melt + RAIN;  // Qw = liquid water available melted/rained at the
                           // snow surface (mm)
  float W_qx = cNode->params[PARAM_SNOW17_PLWHC] *
               cNode->states[STATE_SNOW17_WI];  // W_qx = liquid water capacity (mm)
  cNode->states[STATE_SNOW17_DEFICIT] = cNode->states[STATE_SNOW17_DEFICIT] + delta_HD_snow +
  ...
  if (cNode->states[STATE_SNOW17_WI] > 0) {
    if ((Qw + cNode->states[STATE_SNOW17_WQ]) >
        ((cNode->states[STATE_SNOW17_DEFICIT] * (1 + cNode->params[PARAM_SNOW17_PLWHC])) +
      ...
      E = 0;
      cNode->states[STATE_SNOW17_WQ] =
          cNode->states[STATE_SNOW17_WQ] + Qw - cNode->states[STATE_SNOW17_DEFICIT];
      cNode->states[STATE_SNOW17_WI] =
          cNode->states[STATE_SNOW17_WI] +
          cNode->states[STATE_SNOW17_DEFICIT];  // W_i increases because water
                                                // refreezes as heat deficit is
                                                // decreased
      cNode->states[STATE_SNOW17_DEFICIT] = 0;
    } else if ((Qw < cNode->states[STATE_SNOW17_DEFICIT])) {  // elseif ((Qw + W_q) <
                                                              // cNode->Deficit)) { //
