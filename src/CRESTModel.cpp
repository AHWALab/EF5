...
bool CRESTModel::WaterBalance(float stepHours, std::vector<float>* precip, std::vector<float>* pet,
...
void CRESTModel::WaterBalanceInt(GridNode* node, CRESTGridNode* cNode, float stepHours,
  ...
  if (precip > adjPET) {
    double precipSoil = (precip - adjPET) * (1 - cNode->params[PARAM_CREST_IM]);  // This is the
                                                                                  // precip that
                                                                                  // makes it to
                                                                                  // the soil
    ...
    if (cNode->states[STATE_CREST_SM] < cNode->params[PARAM_CREST_WM]) {
      ...
      double A = Wmaxm * (1 - pow(1 - cNode->states[STATE_CREST_SM] / cNode->params[PARAM_CREST_WM],
                                  1 / (1 + cNode->params[PARAM_CREST_B])));
      if (precipSoil + A >= Wmaxm) {
        R = precipSoil - (cNode->params[PARAM_CREST_WM] -
        ...
        double infiltration = cNode->params[PARAM_CREST_WM] *
                              (pow(1 - A / Wmaxm, 1 + cNode->params[PARAM_CREST_B]) -
                               pow(1 - (A + precipSoil) / Wmaxm, 1 + cNode->params[PARAM_CREST_B]));
    ...
    temX = (cNode->states[STATE_CREST_SM] + Wo) / cNode->params[PARAM_CREST_WM] / 2 *
           (cNode->params[PARAM_CREST_FC] * stepHours);  // Calculate how much water can infiltrate
    ...
    double ExcessET =
        (adjPET - precip) * cNode->states[STATE_CREST_SM] / cNode->params[PARAM_CREST_WM];
  ...
  for (size_t i = 0; i < numNodes; i++) {
    ...
    if (!paramGrids->at(PARAM_CREST_IWU)) {
      cNode->states[STATE_CREST_SM] =
          cNode->params[PARAM_CREST_IWU] * cNode->params[PARAM_CREST_WM] / 100.0;
    }
