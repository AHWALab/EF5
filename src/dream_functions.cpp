...
void InitVar(struct DREAM_Parameters* pstPar, struct DREAM_Variables** pstRUN,
  ...
  *pstOutput = (struct DREAM_Output*)malloc(sizeof(struct DREAM_Output));
  ...
  allocate2D(&(*pstOutput)->R_stat, floorf((*pstRUN)->Nelem / pstPar->steps) + buffer,
             pstPar->n + 1);
  ...
  (*pstRUN)->Sequences =
      (float***)malloc(floorf(1.25 * (float)((*pstRUN)->Nelem)) * sizeof(float**));
...
void Gelman(float** R_Stat, int R_Stat_Index, float*** Sequences, int numSamples, int numParams,
            int numChains, int start_loc) {
  ...
  if (numSamples < 10) {
    ...
    for (iParams = 0; iParams < numParams; iParams++) {
      sigma2[iParams] =
          ((numSamplesf - 1) / numSamplesf) * W[iParams] + (1 / numSamplesf) * B[iParams];
    }
...
void offde(float** x_new, float** x_old, float** X, float** CR, struct DREAM_Parameters* MCMC,
...
void ReflectBounds(float** x_new, struct Model_Input* Input, int nmbOfIndivs, int Dim) {
  ...
  for (i = 0; i < nmbOfIndivs; i++) {
    for (j = 0; j < Dim; j++) {
      ...
      if (x_new[i][j] < Input->ParRangeMin[j]) {
        x_new[i][j] =
            Input->ParRangeMin[j] + UNIFORM_RAND * (Input->ParRangeMax[j] - Input->ParRangeMin[j]);
      }
...
void metrop(float** newgen, float* alpha, float* accept, float** x, float** p_x, float* log_p_x,
...
void CalcDelta(float* delta_tot, struct DREAM_Parameters* MCMC, float* delta_normX,
               struct DREAM_Variables* RUNvar, int gnum) {
...
void AdaptpCR(struct DREAM_Variables* RUNvar, struct DREAM_Parameters* MCMC, float* delta_tot) {
  ...
  for (i = 0; i < MCMC->nCR; i++) {
    ...
    INFO_LOGF("pCR %i %f %f %f %f", i, RUNvar->pCR[0][i], delta_tot[i], RUNvar->lCR[i],
              sumarray(&delta_tot[0], MCMC->nCR, 1));
  }
...
void RemOutLierChains(float** X, struct DREAM_Variables* RUNvar, struct DREAM_Parameters* MCMC,
  ...
  MEMORYCHECK(mean_hist_logp,
              "ERROR at RemOutLierChains: Out of Memory!! "
  ...
  if (strcmp(MCMC->outlierTest, "Grubbs_test") == 0) {
    printf(
        "ERROR at RemOutLierChains: Grubbs_test option has not been "
        "implemented\n");
    exit(1);
  }
  ...
  if (strcmp(MCMC->outlierTest, "Mahal_test") == 0) {
    printf(
        "ERROR at RemOutLierChains: Mahal_test option has not been "
        "implemented\n");
    exit(1);
  }
