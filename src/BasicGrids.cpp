...
// *loc);
static GaugeConfigSection* FindGauge(std::map<unsigned long, GaugeConfigSection*>* gauges,
                                     GridNode*                                     node);
...
static int  FlowsIn(long nextX, long nextY, FLOW_DIR dir, long currentHeight, GridLoc* locIn,
                    long maxSearch, GridLoc* locOut);
...
FLOW_DIR reverseDir[] = {FLOW_WEST,      FLOW_SOUTHWEST, FLOW_SOUTH,     FLOW_SOUTHEAST, FLOW_EAST,
                         FLOW_NORTHEAST, FLOW_NORTH,     FLOW_NORTHWEST, FLOW_QTY};
...
bool LoadBasicGrids() {
  ...
  if (g_basicConfig->IsESRIDDM()) {
    if (CheckESRIDDM()) {
      ...
      ERROR_LOG(
          "Was expecting an ESRI Drainage Direction Map and got invalid "
          "values!");
      return false;
    }
  } else {
    if (!CheckSimpleDDM()) {
      ERROR_LOG(
          "Was expecting a simple Drainage Direction Map and got invalid "
          "values!");
      return false;
    }
  }
...
void FindIndBasins(float left, float right, float top, float bottom, const char* output) {
  ...
  for (FAMSearch* currentFS = gridCells.front(); currentFS != NULL; currentFS = gridCells.front()) {
    ...
    fprintf(test, "[Gauge %i] cellx=%li celly=%li outputts=false #Num Cells = %f\n", totalGauges,
            cCell->x, cCell->y, g_FAM->data[cCell->y][cCell->x]);
    printf("Gauge %i cellx=%li celly=%li basinarea=%f\n", totalGauges, cCell->x, cCell->y,
           g_FAM->data[cCell->y][cCell->x]);
    totalGauges++;
    ...
    while (!walkNodes.empty()) {
      ...
      for (int i = 0; i < FLOW_QTY; i++) {
        if (TestUpstream(cCell->x, cCell->y, (FLOW_DIR)i, &locN)) {
          ...
          if (index < gcItrs.size() && g_DEM->data[nextN->y][nextN->x] != g_DEM->noData &&
...
void ClipBasicGrids(BasinConfigSection* basin, std::vector<GridNode>* nodes, const char* name,
                    const char* output) {
  ...
  for (std::vector<GridNode>::iterator itr = nodes->begin(); itr != nodes->end(); itr++) {
    ...
    for (int dist = 1; dist < maxDist; dist++) {
      ...
      if (testX < g_FAM->numCols && testY < g_FAM->numRows &&
          g_FAM->data[testY][testX] != g_FAM->noData && g_FAM->data[testY][testX] > maxFlow &&
          g_DEM->data[node->y][node->x] == g_DEM->data[testY][testX]) {
        float thisDist = powf(testY - node->y, 2.0) + powf(testX - node->x, 2.0);
        if (thisDist < minDist) {
          minDist = thisDist;
...
void CarveBasin(BasinConfigSection* basin, std::vector<GridNode>* nodes,
                std::map<GaugeConfigSection*, float*>* inParamSettings,
                std::map<GaugeConfigSection*, float*>* outParamSettings, GaugeMap* gaugeMap,
                float* defaultParams, std::map<GaugeConfigSection*, float*>* inRouteParamSettings,
  ...
  for (std::vector<GaugeConfigSection*>::iterator itr = gauges->begin(); itr != gauges->end();) {
    ...
    if (gauge->HasObsFlowAccum()) {
      ...
      float   testError,
          minError = pow(
              (float)(g_FAM->data[loc->y][loc->x]) - gauge->GetObsFlowAccum() * areaFactor, 2.0);
      minLoc.x = loc->x;
      minLoc.y = loc->y;
      long testX, testY;
      for (int dist = 1; dist < maxDist; dist++) {
        ...
        if (testX < g_FAM->numCols && g_FAM->data[testY][testX] != g_FAM->noData) {
          testError =
              pow((float)(g_FAM->data[testY][testX]) - gauge->GetObsFlowAccum() * areaFactor, 2.0);
    ...
    if (g_DEM->data[loc->y][loc->x] == g_DEM->noData ||
        g_FAM->data[loc->y][loc->x] == g_FAM->noData || g_FAM->data[loc->y][loc->x] < 0) {
      ERROR_LOGF("Gauge \"%s\" is located in a no data grid cell!", gauge->GetName());
      return;
    }
    gauge->SetFlowAccum(g_FAM->data[loc->y][loc->x]);
    gauge->SetUsed(false);
    INFO_LOGF("Gauge %s (%f, %f; %ld, %ld): FAM %ld", gauge->GetName(), gauge->GetLat(),
              gauge->GetLon(), loc->y, loc->x, gauge->GetFlowAccum());
    itr++;
  }
  ...
  for (GaugeConfigSection* currentGauge = (*gauges)[0]; currentGauge != NULL;
    ...
    if (pitr == inParamSettings->end() && !defaultParams) {
      ERROR_LOGF("Independent basin \"%s\" lacks a water balance parameter set!",
    ...
    if (inRouteParamSettings) {
      ...
      if (pitr == inRouteParamSettings->end()) {
        outRouteParamSettings->insert(
            std::pair<GaugeConfigSection*, float*>(currentGauge, defaultRouteParams));
      } else {
        outRouteParamSettings->insert(
    ...
    if (inSnowParamSettings) {
      ...
      if (pitr == inSnowParamSettings->end()) {
        outSnowParamSettings->insert(
            std::pair<GaugeConfigSection*, float*>(currentGauge, defaultSnowParams));
      } else {
        outSnowParamSettings->insert(
            std::pair<GaugeConfigSection*, float*>(pitr->first, pitr->second));
      }
    ...
    if (inInundationParamSettings) {
      ...
      if (pitr == inInundationParamSettings->end()) {
        outInundationParamSettings->insert(
            std::pair<GaugeConfigSection*, float*>(currentGauge, defaultInundationParams));
      } else {
        outInundationParamSettings->insert(
            std::pair<GaugeConfigSection*, float*>(pitr->first, pitr->second));
      }
    ...
    if (GetDownstreamHeight(currentN->x, currentN->y, &outsideHeight)) {
      currentN->horLen = g_Projection->GetLen(currentN->refLoc.x, currentN->refLoc.y,
      ...
      if (nodeGauge) {
        ...
        if (inRouteParamSettings) {
          ...
          if (pitr == inRouteParamSettings->end()) {
            ...
            outRouteParamSettings->insert(
                std::pair<GaugeConfigSection*, float*>(nodeGauge, pitr->second));
          } else {
            outRouteParamSettings->insert(
                std::pair<GaugeConfigSection*, float*>(pitr->first, pitr->second));
          }
      ...
      if (keepGoing) {
        for (int i = 1; i < FLOW_QTY; i++) {
          if (TestUpstream(currentN, (FLOW_DIR)i, &nextNode) && currentNode < totalAccum) {
            ...
            float DEMDiff  = (float)(nextNDEM - currentNDEM);  // Upstream (higher elevation)
    ...
    INFO_LOGF("Walked %lu (out of %lu) nodes for %s!", (unsigned long)currentNode,
              (unsigned long)totalAccum, currentGauge->GetName());
  }
  ...
  for (long i = nodes->size() - 1; i >= 0; i--) {
    ...
    if (node->downStreamNode != INVALID_DOWNSTREAM_NODE) {
      ...
      if (nodes->at(node->downStreamNode).relief < node->relief) {  // relief fix
...
void AssignBasinParams(
    BasinConfigSection* basin, std::map<GaugeConfigSection*, float*>* inParamSettings,
    std::map<GaugeConfigSection*, float*>* outParamSettings, float* defaultParams,
    std::map<GaugeConfigSection*, float*>* inRouteParamSettings,
    std::map<GaugeConfigSection*, float*>* outRouteParamSettings, float* defaultRouteParams,
    std::map<GaugeConfigSection*, float*>* inSnowParamSettings,
    std::map<GaugeConfigSection*, float*>* outSnowParamSettings, float* defaultSnowParams,
    std::map<GaugeConfigSection*, float*>* inInundationParamSettings,
    std::map<GaugeConfigSection*, float*>* outInundationParamSettings,
    float*                                 defaultInundationParams) {
  std::vector<GaugeConfigSection*>*               gauges = basin->GetGauges();
  ...
  for (size_t g = 0; g < gauges->size(); g++) {
    ...
    if (pitr == inParamSettings->end()) {
      if (defaultParams) {
        outParamSettings->insert(std::pair<GaugeConfigSection*, float*>(gauge, defaultParams));
      }
    ...
    if (inRouteParamSettings) {
      ...
      if (pitr == inRouteParamSettings->end()) {
        if (defaultRouteParams) {
          outRouteParamSettings->insert(
              std::pair<GaugeConfigSection*, float*>(gauge, defaultRouteParams));
        }
      } else {
        outRouteParamSettings->insert(
            std::pair<GaugeConfigSection*, float*>(pitr->first, pitr->second));
    ...
    if (inSnowParamSettings) {
      ...
      if (pitr == inSnowParamSettings->end()) {
        if (defaultSnowParams) {
          outSnowParamSettings->insert(
              std::pair<GaugeConfigSection*, float*>(gauge, defaultSnowParams));
        }
      } else {
        outSnowParamSettings->insert(
            std::pair<GaugeConfigSection*, float*>(pitr->first, pitr->second));
    ...
    if (inInundationParamSettings) {
      ...
      if (pitr == inInundationParamSettings->end()) {
        if (defaultInundationParams) {
          outInundationParamSettings->insert(
              std::pair<GaugeConfigSection*, float*>(gauge, defaultInundationParams));
        }
      } else {
        outInundationParamSettings->insert(
            std::pair<GaugeConfigSection*, float*>(pitr->first, pitr->second));
...
bool TestUpstream(long nextX, long nextY, FLOW_DIR dir, GridLoc* loc) {
  ...
  if (nextX >= 0 && nextY >= 0 && nextX < g_DDM->numCols && nextY < g_DDM->numRows &&
      g_DDM->data[nextY][nextX] == wantDir) {  // && g_FAM->data[nextY][nextX] <= currentFAC) {
    loc->x = nextX;
    loc->y = nextY;
    return true;
  }
...
GaugeConfigSection* FindGauge(std::map<unsigned long, GaugeConfigSection*>* gauges,
                              GridNode*                                     node) {
    ...
    for (long col = 0; col < g_DDM->numCols; col++) {
      ...
      switch ((int)(g_DDM->data[row][col])) {
        ...
        case 128:
          continue;
        default:
          ERROR_LOGF("Bad DDM value %i at (%li, %li) %f", (int)(g_DDM->data[row][col]), col, row,
                     g_DDM->noData);
          return false;
      }
    }
...
int FlowsIn(long nextX, long nextY, FLOW_DIR dir, long currentHeight, GridLoc* locIn,
            long maxSearch, GridLoc* locOut) {
  switch (dir) {
    case FLOW_NORTH:
      nextY--;
        ...
        while (!walkNodes.empty()) {
          ...
          for (int i = 1; i < FLOW_QTY; i++) {
            ...
            if (FlowsIn(currentN->x, currentN->y, (FLOW_DIR)i,
                        g_DEM->data[currentN->y][currentN->x], &locIn, 5, &locOut)) {
