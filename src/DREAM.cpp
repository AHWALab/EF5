...
void DREAM::Initialize(CaliParamConfigSection*        caliParamConfigNew,
  ...
  INFO_LOGF("num params is %i (water balance :%i, routing: %i, snow: %i)", numParams, numParamsWB,
            numParamsR, numParamsS);
  ...
  memcpy(&(maxParams[numParamsWB]), routingCaliParamConfig->GetParamMaxs(),
         sizeof(float) * numParamsR);
  if (numParamsS > 0) {
    memcpy(&(minParams[numParamsWB + numParamsR]), snowCaliParamConfig->GetParamMins(),
           sizeof(float) * numParamsS);
    memcpy(&(maxParams[numParamsWB + numParamsR]), snowCaliParamConfig->GetParamMaxs(),
           sizeof(float) * numParamsS);
  }
  ...
  float newdraw =
      floorf((float)pointerMCMC->ndraw / (float)(pointerMCMC->seq * pointerMCMC->steps) + 0.5) *
...
void DREAM::Initialize(CaliParamConfigSection* caliParamConfigNew, int numParamsNew,
...
void DREAM::CalibrateParams() {
  ...
  // Allocation-----------------------------------//
  MEMORYCHECK(pointerRUNvar,
              "at dream.c: Memory Allocation for DREAM struct "
              "run variable not successfull\n");
  MEMORYCHECK(pointerRUNvar->hist_logp,
              "at dream.c: Memory Allocation for "
              "DREAM run variable hist_logp not "
              "successfull\n");
  MEMORYCHECK(pointerRUNvar->pCR,
              "at dream.c: Memory Allocation for DREAM run "
              "variable pCR not successfull\n");
  MEMORYCHECK(pointerRUNvar->CR,
              "at dream.c: Memory Allocation for DREAM run "
              "variable CR not successfull\n");
  MEMORYCHECK(pointerRUNvar->lCR,
              "at dream.c: Memory Allocation for DREAM run "
              "variable lCR not successfull\n");
  MEMORYCHECK(pointerOutput,
              "at dream.c: Memory Allocation for DREAM struct "
              "output variable not successfull\n");
  MEMORYCHECK(pointerOutput->AR,
              "at dream.c: Memory Allocation for DREAM "
              "output variable AR not successfull\n");
  MEMORYCHECK(pointerOutput->CR,
              "at dream.c: Memory Allocation for DREAM "
              "output variable CR not successfull\n");
  MEMORYCHECK(pointerOutput->outlier,
              "at dream.c: Memory Allocation for DREAM "
              "output variable outlier not "
              "successfull\n");
  MEMORYCHECK(pointerRUNvar->Sequences,
              "at dream.c: Memory Allocation for "
  ...
  LHSU(&x, pointerInput->nPar, pointerInput->ParRangeMax, pointerInput->ParRangeMin,
       pointerMCMC->seq);
  ...
  MEMORYCHECK(log_p,
              "at dream.c: Memory Allocation for DREAM variable log_p "
              "not successfull\n");
  ...
  MEMORYCHECK(delta_tot,
              "at dream.c: Memory Allocation for DREAM variable "
  ...
  Gelman(pointerOutput->R_stat, 0, pointerRUNvar->Sequences, pointerRUNvar->iloc, pointerMCMC->n,
         pointerMCMC->seq, 0);

  // Allocate Variables
  allocate2D(&x_old, pointerMCMC->seq, pointerMCMC->n);
  MEMORYCHECK(x_old,
              "at dream.c: Memory Allocation for DREAM variable x_old "
              "not successfull\n");
  p_old = (float*)malloc(pointerMCMC->seq * sizeof(float*));
  MEMORYCHECK(p_old,
              "at dream.c: Memory Allocation for DREAM variable p_old "
              "not successfull\n");
  log_p_old = (float*)malloc(pointerMCMC->seq * sizeof(float*));
  MEMORYCHECK(log_p_old,
              "at dream.c: Memory Allocation for DREAM variable "
              "log_p_old not successfull\n");
  allocate2D(&x_new, pointerMCMC->seq, pointerMCMC->n);
  MEMORYCHECK(x_new,
              "at dream.c: Memory Allocation for DREAM variable x_new "
              "not successfull\n");
  allocate2D(&p_xnew, pointerMCMC->seq, 2);
  MEMORYCHECK(p_xnew,
              "at dream.c: Memory Allocation for DREAM variable p_xnew "
              "not successfull\n");
  log_p_xnew = (float*)malloc(pointerMCMC->seq * sizeof(float));
  MEMORYCHECK(log_p_xnew,
              "at dream.c: Memory Allocation for DREAM variable "
  ...
  MEMORYCHECK(accept,
              "at dream.c: Memory Allocation for DREAM variable accept "
              "not successfull\n");

  // Preallocate memory needed here
  allocate2D(&newgen, pointerMCMC->seq, pointerMCMC->n + 2);
  MEMORYCHECK(newgen,
              "at dream.c: Memory Allocation for DREAM variable newgen "
              "not successfull\n");
  ...
  while ((pointerRUNvar->Iter < pointerMCMC->ndraw) && !converged) {
    ...
    for (gen_number = 0; gen_number < pointerMCMC->steps; gen_number++) {
      ...
      // of X
      offde(x_new, x_old, X, pointerRUNvar->CR, pointerMCMC, pointerRUNvar->Table_JumpRate,
            pointerInput, "Reflect", R2, "No");
      ...
      metrop(newgen, alpha12, accept, x_new, p_xnew, log_p_xnew, x_old, p_old, log_p_old,
             pointerInput, pointerMCMC, 3);
      ...
      pointerOutput->AR[pointerRUNvar->counter - 1][1] =
          100.0 * (sumarray(accept, pointerMCMC->seq, 1) / (float)((pointerMCMC->seq + ItExtra)));
    ...
    Gelman(pointerOutput->R_stat, pointerRUNvar->teller - 1, pointerRUNvar->Sequences,
           (end_loc - start_loc + 1), pointerMCMC->n, pointerMCMC->seq, start_loc);
...
void DREAM::WriteOutput(char* outputFile, MODELS model, ROUTES route, SNOWS snow) {
  ...
  if (g_interpolationUsed) {
    fprintf(file,
            "***Interpolation of observed data was applied during calibration due to mismatched "
            "time steps or missing values in the observed dataset***\n\n");
  } else if (g_mismatchedFrequencies) {
    fprintf(file,
            "***Calibration was performed using observed data that either contains missing values "
            "or has a frequency that does not match the time steps specified in the control file. "
            "Consider setting INTERPOLATE_OBS=True to interpolate the observed data***\n\n");
  }
  ...
  // Print best parameters at the bottom
...
void DREAM::CompDensity(float** p, float* log_p, float** x, struct DREAM_Parameters* MCMC,
