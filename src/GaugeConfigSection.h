...
class GaugeConfigSection : public ConfigSection {
 public:
  ...
  bool ContinueUpstream() {
    return continueUpstream;
  }
  long GetFlowAccum() {
    return flowAccum;
  }
  bool HasObsFlowAccum() {
    return obsFlowAccumSet;
  }
  float GetObsFlowAccum() {
    return obsFlowAccum;
  }
  GridLoc* GetGridLoc() {
    return &gridLoc;
  ...
  void SetLon(float newVal) {
    lon = newVal;
  }
  void SetCellX(long newX) {
    gridLoc.x = newX;
  }
  void SetCellY(long newY) {
    gridLoc.y = newY;
  }
  void SetUsed(bool newVal) {
    used = newVal;
  }
  void SetFlowAccum(long newVal) {
    flowAccum = newVal;
  }
  bool NeedsProjecting() {
    return latSet;
  }
