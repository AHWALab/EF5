...
bool HyMOD::InitializeModel(std::vector<GridNode>*                 newNodes,
...
bool HyMOD::WaterBalance(float stepHours, std::vector<float>* precip, std::vector<float>* pet,
...
void HyMOD::WaterBalanceInt(GridNode* node, HyMODGridNode* cNode, float stepHours, float precipIn,
                            float petIn) {
  float precip =
      precipIn * stepHours * cNode->params[PARAM_HYMOD_PRECIP];  // precipIn is mm/hr, precip is mm
  float pet = petIn * stepHours;                                 // petIn in mm/hr, pet is mm

  float Cbeg = cNode->CPar * (1 - pow(1 - (cNode->XHuz / cNode->params[PARAM_HYMOD_HUZ]),
                                      1 + cNode->b));  // Contents at begining
  float OV2  = std::max(
      0.0f, precip + cNode->XHuz - cNode->params[PARAM_HYMOD_HUZ]);  // Compute OV2 if enough PP
  ...
  float Hend  = cNode->params[PARAM_HYMOD_HUZ] *
                (1 - pow(1 - Cend / cNode->CPar,
...
void HyMOD::InitializeParameters(std::map<GaugeConfigSection*, float*>* paramSettings) {
  ...
  for (std::vector<GridNode>::iterator itr = nodes->begin(); itr != nodes->end(); itr++) {
    ...
    cNode->XHuz = cNode->params[PARAM_HYMOD_HUZ] *
