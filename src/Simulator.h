...
class Simulator {
 public:
  ...
  void PreloadForcingsMemoryOnly();
  bool LoadSavedForcings(char* file, bool cali);
  void SaveForcings(char* file);

  void                             ShareForcingsFrom(Simulator* other);
  std::vector<std::vector<float>>* sharedPrecipCali;
  std::vector<std::vector<float>>* sharedPETCali;
  std::vector<std::vector<float>>* sharedTempCali;

  void   CleanUp();
  void   BasinAvg(const char* inputDir);
  void   BasinAvgPrecip();
  void   Simulate(bool trackPeaks = false);
  bool   InitializeShared(TaskConfigSection* taskN, const std::vector<GridNode>& sharedNodes,
                          const GaugeMap& sharedGaugeMap);
  float  SimulateForCali(float* testParams);
  float* SimulateForCaliTS(float* testParams);
  float* GetObsTS();
  size_t GetNumSteps() {
    return totalTimeStepsOutsideWarm;
  }
  const std::vector<GridNode>& GetNodes() const {
    return nodes;
  }
  const GaugeMap& GetGaugeMap() const {
    return gaugeMap;
  }
  int GetMissingQPE() const {
    return missingQPE;
  }
  int GetMissingQPF() const {
    return missingQPF;
  }

 private:
  bool InitializeBasic(TaskConfigSection* task);
  bool InitializeBasicFromShared(TaskConfigSection* task, const std::vector<GridNode>& sharedNodes,
                                 const GaugeMap& sharedGaugeMap);
  ...
  float ComputeThresValueP(float discharge, float action, float actionSD, float minor,
  ...
  TimeUnit *timeStep, *timeStepSR, *timeStepLR, *timeStepPrecip, *timeStepQPF, *timeStepPET,
      *timeStepTemp, *timeStepTempF;
  float   precipConvert, qpfConvert, petConvert, timeStepHours, timeStepHoursLR;
  TimeVar currentTime, currentTimePrecip, currentTimeQPF, currentTimePET, currentTimeTemp,
      currentTimeTempF, beginTime, endTime, warmEndTime, beginLRTime;
  DatedName *precipFile, *qpfFile, *petFile, *tempFile, *tempFFile, currentTimeText,
      currentTimeTextOutput;
  std::vector<float> currentFF, currentSF, currentQ, avgPrecip, avgPET, avgSWE, currentSWE, avgT,
      avgSM, avgFF, avgSF, currentDepth, computeVec;
